...
const createTable = async (req, res) => {
    ...
    try {
        let newTable = await prisma.questionaryOptionTable.create({
            data: {
                text: body.text,
                hint: body?.hint,
        ...
        const questionaryOptionTables =
            await prisma.questionaryOptionTable.findMany({
                orderBy: {
                    order: "asc",
                },
            });
        ...
        await updateModelOrder({
            modelsToReorder: questionaryOptionTables,
            newItem: newTable,
            newOrder: body?.order,
            modelName: "questionaryOptionTable",
        });
...
const updateTable = async (req, res) => {
    try {
        ...
        const tariffQuery = body?.tariffes?.map((tariff) => ({ id: tariff }));
        let newTable = await prisma.questionaryOptionTable.update({
            where: {
                id: body.id,
            },
            data: {
...
const deleteTable = async (req, res) => {
    ...
    try {
        const deletedRoom = await prisma.questionaryOptionTable.delete({
            where: {
                id: body.id,
            },
        });
